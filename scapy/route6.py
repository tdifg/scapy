...
class Route6:
    ...
    def __repr__(self):
        ...
        for net, msk, gw, iface, cset in self.routes:
            rtlst.append(('%s/%i' % (net, msk), gw, (iface if isinstance(iface, basestring)
                                                     else iface.name), ", ".join(cset) if len(cset) > 0 else ""))
    ...
    def make_route(self, dst, gw=None, dev=None):
        ...
        if dev is None:
            ...
            ifaddr = construct_source_candidate_set(
                prefix, plen, devaddrs, LOOPBACK_INTERFACE)
    ...
    def route(self, dst, dev=None):
        ...
        if dev is not None:
            k = dst + "%%" + \
                (dev if isinstance(dev, basestring) else dev.pcap_name)
        if k in self.cache:
            return self.cache[k]
