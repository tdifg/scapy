    ...
    def read_block_idb(self, block, _):
        ...
        while len(options) >= 4:
            ...
            if code == 0:
                if length != 0:
                    warning(
                        "PcapNg: invalid option length %d for end-of-option" % length)
                break
            if length % 4:
                length += (4 - (length % 4))
...
nano:       use nanosecond-precision (requires libpcap >= 1.5.0)
    ...
    def _write_packet(self, packet, sec=None, usec=None, caplen=None, wirelen=None):
        ...
        if sec is None or usec is None:
            ...
            if usec is None:
                usec = int(
                    round((t - it) * (1000000000 if self.nano else 1000000)))
...
def pretty_routes(rtlst, header, sortBy=0):
    ...
    # TODO: find a better and more precise way of doing this. That's currently
    # working but very complicated
    width = get_terminal_width()
    if width:
        if sum(colwidth) > width:
            # Needs to be cropped
            # Windows has a fat window border
            ...
            def _crop(x, width):
                ...
                if _r != x:
                    ...
                    return _r + "..."
                return _r
            rtlst = [tuple([_crop(rtlst[j][i], colwidth[i]) for i in range(
                0, len(rtlst[j]))]) for j in range(0, len(rtlst))]
...
def whois(ip_address):
    ...
    lines = [line for line in answer.split("\n") if not line or (
        line and not line.startswith(ignore_tag))]
