...
class SkinnyDateTimeField(StrFixedLenField):
    ...
    def m2i(self, pkt, s):
        year, month, dow, day, hour, min, sec, milisecond = struct.unpack(
            '<8I', s)
    ...
    def h2i(self, pkt, s):
        ...
        if type(s) is str:
            ...
            if not s:
                y, m, d, h, min, sec, rest, rest, rest = time.gmtime(
                    time.time())
                t = (y, m, d, h, min, sec)
            else:
                t = s
...
_skinny_message_callinfo_restrictions = ['CallerName', 'CallerNumber', 'CalledName', 'CalledNumber', 'OriginalCalledName',
                                         'OriginalCalledNumber', 'LastRedirectName', 'LastRedirectNumber'] + ['Bit%d' % i for i in xrange(8, 15)]
...
class SkinnyMessageCallInfo(Packet):
    ...
    fields_desc = [StrFixedLenField("callername", "Jean Valjean", 40),
                   ...
                   FlagsField('restriction', 0, 16,
                              _skinny_message_callinfo_restrictions),
