...
def create_oxm_cls():
    ...
    cls = type(cls_name, (Packet,), {
               "name": oxm_name, "fields_desc": oxm_fields})
...
class OXMPacketListField(PacketListField):
    ...
    def __init__(self, name, default, cls, length_from=None, autocomplete=prereq_autocomplete):
        PacketListField.__init__(
            self, name, default, cls, length_from=length_from)
        ...
        if self.autocomplete:
            ...
            for oxm in fix_val:
                ...
                while f in need_prereq:
                    ...
                    # you can still go manual by setting
                    # prereq_autocomplete=False
        return val
...
class OFPETExperimenter(_ofp_header):
    ...
    fields_desc = [ByteEnumField("version", 0x04, ofp_version),
                   ...
                   IntField("xid", 0),
                   ShortEnumField(
        "errtype", "OFPET_EXPERIMENTER", ofp_error_type),
...
class OFPTPacketOut(_ofp_header):
    ...
    fields_desc = [ByteEnumField("version", 0x04, ofp_version),
                   ...
                   FieldLenField("actions_len", None, fmt="H",
                                 length_of="actions"),
                   XBitField("pad", 0, 48),
                   ActionPacketListField("actions", [], Packet,
...
ofp_action_types_flags = ofp_action_types.values()[:-1]
...
class OFPMPReplyGroupFeatures(_ofp_header):
    ...
    fields_desc = [ByteEnumField("version", 0x04, ofp_version),
                   ...
                   FlagsField("actions_select", 0, 32,
                              ofp_action_types_flags),
                   FlagsField("actions_indirect", 0, 32,
                              ofp_action_types_flags),
...
class TableFeaturesPropPacketListField(PacketListField):
    ...
    def getfield(self, pkt, s):
        ...
        while remain and len(remain) >= 4:
            l = TableFeaturesPropPacketListField._get_table_features_prop_length(
                remain)
            # add padding !
            lpad = l + (8 - l % 8) % 8
...
class OFPTGetAsyncReply(_ofp_header):
    ...
    fields_desc = [ByteEnumField("version", 0x04, ofp_version),
                   ...
                   ShortField("len", 32),
                   IntField("xid", 0),
                   FlagsField("packet_in_mask_master", 0,
                              32, ofp_packet_in_reason),
                   FlagsField("packet_in_mask_slave", 0,
                              32, ofp_packet_in_reason),
                   FlagsField("port_status_mask_master",
                              0, 32, ofp_port_reason),
                   FlagsField("port_status_mask_slave",
                              0, 32, ofp_port_reason),
                   FlagsField("flow_removed_mask_master", 0,
                              32, ofp_flow_removed_reason),
...
# ofpt_cls allows generic method OpenFlow() to choose the right class for
# dissection
