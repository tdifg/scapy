...
#########################################################################
# Commented commands are already in HPAVTypeList, the other have to be
# implemted
...
# TODO: The complete list of Padding can help to improve the condition in
# VendorMME Class
EofPadList = [0xA000, 0xA038]
...
class GetDeviceVersion(Packet):
    ...
    fields_desc = [ByteEnumField("Status", 0x0, StatusCodes),
                   ...
                   FieldLenField("VersionLen", None,
                                 count_of="DeviceVersion", fmt="B"),
                   StrLenField("DeviceVersion", b"NoVersion\x00",
                               length_from=lambda pkt: pkt.VersionLen),
                   StrLenField("DeviceVersion_pad", b"\xcc\xcc\xcc\xcc\xcc" +
                               b"\x00" * 59, length_from=lambda pkt: 64 - pkt.VersionLen),
...
class NetworkInfoConfirmationV10(Packet):
    ...
    fields_desc = [XByteField("LogicalNetworksNumber", 0x01),
                   PacketListField("NetworksInfos", "", NetworkInfoV10,
                                   length_from=lambda pkt: pkt.LogicalNetworksNumber * 17),
...
class NetworkInfoConfirmationV11(Packet):
    ...
    fields_desc = [StrFixedLenField("reserved_n1", b"\x00\x00\x3a\x00\x00", 5),
                   ...
                   PacketListField("NetworksInfos", "", NetworkInfoV11,
                                   length_from=lambda pkt: pkt.LogicalNetworksNumber * 26),
                   XByteField("StationsNumber", 0x01),
                   StrFixedLenField(
                       "reserverd_s1", b"\x00\x00\x00\x00\x00", 5),
...
class QUAResetFactoryConfirm(Packet):
    ...
    # TODO : Probably a Status bytefield?
    fields_desc = [ByteEnumField("Status", 0x0, StatusCodes)]
...
class SnifferIndicate(Packet):
    # TODO: Some bitfield have been regrouped for the moment => need more work
    # on it
...
class ReadMACMemoryConfirmation(Packet):
    ...
    fields_desc = [ByteEnumField("Status", 0x00, ReadMACStatus),
                   ...
                   FieldLenField("MACLen", None,
                                 length_of="MACData", fmt="<H"),
                   StrLenField("MACData", b"\x00",
                               length_from=lambda pkt: pkt.MACLen),
                   ]
...
class ReadModuleDataConfirmation(Packet):
    ...
    fields_desc = [ByteEnumField("Status", 0x0, StatusCodes),
                   ...
                   FieldLenField("DataLen", None,
                                 count_of="ModuleData", fmt="<H"),
                   LEIntField("Offset", 0x00000000),
                   LEIntField("checksum", None),
                   StrLenField("ModuleData", b"\x00",
...
class AutoConnection(Packet):
    ...
    fields_desc = [XByteField("Action", 0x00),
                   ...
                   PacketListField("ClassifierObjs", "",
                                   ClassifierObj, length_from=lambda x: 24),
...
class ReadConfBlockConfirmation(Packet):
    ...
    fields_desc = [ByteEnumField("Status", 0x00, ReadConfBlockCodes),
                   FieldLenField("BlockLen", None,
                                 count_of="ConfigurationBlock", fmt="B"),
...
bind_layers(HomePlugAV, NetworkInfoConfirmationV10,
            {"HPtype": 0xA039, "version": 0x00})
bind_layers(HomePlugAV, NetworkInfoConfirmationV11,
            {"HPtype": 0xA039, "version": 0x01})
bind_layers(NetworkInfoConfirmationV10, NetworkInfoV10,
            {"HPtype": 0xA039, "version": 0x00})
bind_layers(NetworkInfoConfirmationV11, NetworkInfoV11,
            {"HPtype": 0xA039, "version": 0x01})
