...
class PNIORealTimeRawData(Packet):
    ...
    fields_desc = [
        StrFixedLenField(
            "load", "", length_from=lambda p: p[PNIORealTimeRawData].length()),
    ]
...
class Profisafe(PNIORealTimeRawData):
    ...
    fields_desc = [
        StrFixedLenField(
            "load", "", length_from=lambda p: p[Profisafe].data_length()),
        XByteField("Control_Status", 0),
        XVarBytesField(
            "CRC", 0, length_from=lambda p: p[Profisafe].crc_length())
    ]
