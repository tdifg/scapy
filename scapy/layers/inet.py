        ...
        if self.chksum is None:
            if isinstance(self.underlayer, IP):
                ...
                psdhdr = struct.pack("!4s4sHH",
                                     ...
                                     ln)
                ck = checksum(psdhdr + p)
                # According to RFC768 if the result checksum is 0, it should be
                # set to 0xFFFF
                if ck == 0:
                    ck = 0xFFFF
...
class TracerouteResult(SndRcvList):
    ...
    def make_graph(self, ASres=None, padding=0):
        ...
        for s, r in self.res:
            r = r.getlayer(IP) or (
                conf.ipv6_enabled and r[scapy.layers.inet6.IPv6]) or r
            s = s.getlayer(IP) or (
                conf.ipv6_enabled and s[scapy.layers.inet6.IPv6]) or s
            ips[r.src] = None
            if TCP in s:
...
#    pkt = IP(dst=target, id=RandShort(), options=b"\x22"*40)/UDP()/load
    pkt = IP(dst=target, id=RandShort(), options=b"\x00" * 40,
...
def fragleak2(target, timeout=0.4, onlyasc=0):
    ...
    try:
        while 1:
            p = sr1(IP(dst=target, options=b"\x00" * 40, proto=200) /
