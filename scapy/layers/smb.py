...
class SMBNegociate_Protocol_Response_Advanced_Security(Packet):
    ...
    fields_desc = [StrFixedLenField("Start", b"\xffSMB", 4),
                   ...
                   # There have been 127490112000000000 tenths of micro-seconds
                   # between 1st january 1601 and 1st january 2005.
                   # 127490112000000000=0x1C4EF94D6228000, so
                   # ServerTimeHigh=0xD6228000 and ServerTimeLow=0x1C4EF94.
...
# When using no security, with EncryptionKeyLength=8, you must have an
# EncryptionKey before the DomainName
...
class SMBSession_Setup_AndX_Request(Packet):
    ...
    fields_desc = [StrFixedLenField("Start", b"\xffSMB", 4),
                   ...
                   StrLenField("ANSIPassword", "Pass",
                               length_from=lambda x: x.ANSIPasswordLength),
...
bind_layers(NBTSession,
            SMBNegociate_Protocol_Request_Header, )
bind_layers(NBTSession,
            SMBNegociate_Protocol_Response_Advanced_Security,  ExtendedSecurity=1)
bind_layers(NBTSession,    SMBNegociate_Protocol_Response_No_Security,
            ExtendedSecurity=0, EncryptionKeyLength=8)
bind_layers(NBTSession,    SMBNegociate_Protocol_Response_No_Security_No_Key,
            ExtendedSecurity=0, EncryptionKeyLength=0)
bind_layers(NBTSession,    SMBSession_Setup_AndX_Request, )
bind_layers(NBTSession,    SMBSession_Setup_AndX_Response, )
bind_layers(SMBNegociate_Protocol_Request_Header,
            SMBNegociate_Protocol_Request_Tail, )
bind_layers(SMBNegociate_Protocol_Request_Tail,
            SMBNegociate_Protocol_Request_Tail, )
