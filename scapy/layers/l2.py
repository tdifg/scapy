...
class EAP_MD5(EAP):
    ...
    fields_desc = [
        ...
        XStrLenField("optional_name", '', length_from=lambda p: 0 if p.len is None or p.value_size is None else (
            p.len - p.value_size - 6))
    ]
...
class EAP_TLS(EAP):
    ...
    fields_desc = [
        ...
        ConditionalField(IntField('tls_message_len', 0),
                         lambda pkt: pkt.L == 1),
        XStrLenField(
            'tls_data', '', length_from=lambda pkt: 0 if pkt.len is None else pkt.len - (6 + 4 * pkt.L))
    ]
...
class EAP_FAST(EAP):
    ...
    fields_desc = [
        ...
        XStrLenField(
            'data', '', length_from=lambda pkt: 0 if pkt.len is None else pkt.len - (6 + 4 * pkt.L))
    ]
...
class LEAP(EAP):
    ...
    fields_desc = [
        ...
        ByteField('version', 1),
        XByteField('unused', 0),
        FieldLenField("count", None, "challenge_response", "B",
                      adjust=lambda p, x: len(p.challenge_response)),
        XStrLenField("challenge_response", "",
                     length_from=lambda p: 0 or p.count),
        StrLenField("username", "", length_from=lambda p: p.len -
                    (8 + (0 or p.count)))
    ]
...
class GRE_PPTP(GRE):
    ...
    def post_build(self, p, pay):
        ...
        if self.payload_len is None:
            pay_len = len(pay)
            p = p[:4] + chr((pay_len >> 8) & 0xff) + \
                chr(pay_len & 0xff) + p[6:]
        return p
